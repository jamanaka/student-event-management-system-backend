...
const restrictToOwnerOrAdmin = (
  ...
  return async (req, res, next) => {
    try {
      ...
      if (!docId) {
        return next(
          new AppError("Document ID is required", 400, "ID_REQUIRED")
        );
      }
      ...
      if (error.name === "CastError") {
        return next(
          new AppError("Invalid document ID format", 400, "INVALID_ID")
        );
      }

      return next(
        new AppError("Permission check failed", 500, "PERMISSION_CHECK_ERROR")
      );
    }
  };
};
...
const validateAdminAction = (allowedActions = [], requireAction = true) => {
  return (req, res, next) => {
    ...
    if (requireAction && !action) {
      return next(
        new AppError("Action parameter is required", 400, "ACTION_REQUIRED")
      );
    }
