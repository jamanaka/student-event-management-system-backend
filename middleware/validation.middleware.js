...
const authValidations = {
    register: validate([
        ...
        body('graduationYear')
            .optional()
            .isInt({ min: 2000, max: 2030 }).withMessage('Graduation year must be between 2000-2030')
    ]),
...
const eventValidations = {
    createEvent: validate([
        ...
        body('category')
            .optional()
            .isIn(['academic', 'social', 'sports', 'cultural', 'career', 'workshop', 'other'])
            .withMessage('Invalid category'),
        
        body('capacity')
            .optional()
            .isInt({ min: 1, max: 1000 }).withMessage('Capacity must be between 1-1000'),
    ...
    updateEventStatus: validate([
        body('status')
            .isIn(['approved', 'rejected', 'cancelled']).withMessage('Invalid status'),
...
const rsvpValidations = {
    createRSVP: validate([
        ...
        body('numberOfGuests')
            .optional()
            .isInt({ min: 0, max: 5 }).withMessage('Guests must be 0-5'),
        
        body('dietaryPreferences')
            .optional()
            .trim()
            .isLength({ max: 200 }).withMessage('Dietary preferences too long')
    ])
};
